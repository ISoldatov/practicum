...
public class SocNet_3 {
    ...
    static class PostsHandler implements HttpHandler {
        ...
        private void handlePostComments(HttpExchange exchange) throws IOException {
            ...
            String jsonComment = new String(inputStream.readAllBytes());
            ...
            try {
                newComment = gson.fromJson(jsonComment, Comment.class);
            ...
            posts.stream().filter(p -> p.getId() == postId).findFirst().ifPresentOrElse(p -> p.addComment(finalNewComment),
                    () -> {
                        try {
