package sprint08.http_server.socnet;
...
public class SocNet {
    ...
    static class PostsHandler implements HttpHandler {
        ...
        private Endpoint getEndpoint(String requestPath, String requestMethod) {
            ...
            String parameter = pathParameters.length == 4 ? pathParameters[2] : null;
            // реализуйте этот метод
            switch (requestMethod) {
                case "GET":
                    if (pathParameters[pathParameters.length - 1].toLowerCase().equals("posts") && pathParameters.length == 2) {
                        return Endpoint.GET_POSTS;
                    }
                    if (pathParameters[pathParameters.length - 1].toLowerCase().equals("comments") && pathParameters.length == 4) {
